...
var welcome = {
    ...
    stimulus: "<p>GoNoGo Task</p> <br><p>Version 3</p><br><br><p> Press any key to begin.</p>",
    on_load : () => {
...
for (let i = 1; i < TOTAL_TRIAL_NUMBER + 1; i++){
    ...
    var fixation = {
        ...
        trial_duration: 500,
        data: {trial_number :Trial_Number, test_part: 'fixation', duration : 500, nextImage : fractal_images[type]},
        on_load: ()=>{
    ...
    var imageAlone = {
        ...
        data: {trial_number :Trial_Number, test_part: 'imageAlone', result : type[0]},
        choices : [37,39], // 37: <-   39:->
        trial_duration : 350,
        //data : {trial_number : Trial_Number, test_part : 'imageAlone'},
        on_load: ()=>{
            // Stimulus from the plug in is hidden
            var x = document.getElementById('jspsych-image-keyboard-response-stimulus');
            x.style.display = "none";

            // // But it's set as the background
            ...
            imageURL = lastTrial.nextImage
            //console.log(lastTrial.nextImage)
            document.body.style.backgroundImage = "url(' " + imageURL + " ')";
            
            // document.body.style.background = url('http://ppcdn.500px.org/75319705/1991f76c0c6a91ae1d23eb94ac5c7a9f7e79c480/2048.jpg')  
    ...
    timeline.push(imageAlone)


    var imageWithLetter = {
        type: "image-keyboard-response",
        stimulus : fractal_images[type],
        trial_duration : 650, // show image for 1000 millisecond
    ...
    var outcome_trial_condition_win = {
        ...
        conditional_function : () =>{
            var data = jsPsych.data.get().filter({'test_part': "fractal_cue"}).last(1).values()[0];
...
var debrief_block = {
    ...
    var rt = Math.round(jsPsych.data.get().filter({'test_part': "fractal_cue"}).select('rt').mean());
    var points = jsPsych.data.get().filter({'test_part': "fractal_cue"}).last(1).values()[0].points;
    //console.log(data);
...
var images = [
    '/images/gonogo_media/S1.bmp',
    '/images/gonogo_media/S2.bmp',
    '/images/gonogo_media/S3.bmp',
    '/images/gonogo_media/S4.bmp'
];
